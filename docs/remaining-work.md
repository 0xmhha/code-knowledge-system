...
| **M3** | T7 — composer causal orchestration (multi-hop `expand_flow`). | [권장] | **Spec approved, but the design PHASE is not finished**: per the spec's own header ("Implementation pending its own plan") and the superpowers spec→plan→impl workflow, the implementation **plan is not yet written** (`docs/superpowers/plans/` has no T7 entry) and implementation is not started. Spec: `docs/superpowers/specs/2026-07-12-t7-causal-chain-orchestration-design.md` (#42). Remaining: ① write the impl plan ② implement (`internal/composer/causal` + `cks.context.get_causal_chain` + composer stage, fixture 19→20). | Avoid clashing with M2 measurement freeze. |
